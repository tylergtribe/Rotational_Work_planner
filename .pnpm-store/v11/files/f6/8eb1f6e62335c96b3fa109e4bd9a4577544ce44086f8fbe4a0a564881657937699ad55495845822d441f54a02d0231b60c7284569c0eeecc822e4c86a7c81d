import { GetterPropType, OutputMode, buildAngularParamsFilterExpression, conventionName, escapeRegExp, generateBodyOptions, generateDependencyImports, generateFormDataAndUrlEncodedFunction, generateMutatorConfig, generateMutatorImports, generateMutatorRequestOptions, generateOptions, generateVerbImports, getAngularFilteredParamsHelperBody, getAngularObjectParamStrategies, getDefaultContentType, getEnumImplementation, getFileInfo, getFullRoute, getImportExtension, getIsBodyVerb, getOperationTagKey, getTagKey, isBoolean, isObject, isOperationInTagBucket, isSyntheticDefaultImportsAllow, jsDoc, makeRouteSafe, pascal, sanitize, toObjectString, upath } from "@orval/core";
...
*/
const buildServiceClassOpen = ({ title, isRequestOptions, isMutator, isGlobalMutator, provideIn, hasQueryParams, hasObjectParams = false }) => {
...
*/
const generateHttpClientImplementation = ({ headers, queryParams, operationName, typeName, response, mutator, body, props, verb, override, formData, formUrlEncoded, paramsSerializer, paramsFilter }, { route: _route, context }) => {
	let route = _route;
	...
	return props.map((prop) => {
		switch (prop.type) {
			...
			case GetterPropType.HEADER: {
				const signalProp = withSignal(prop, { hasDefault: prop.type === GetterPropType.PARAM ? paramDefaults.get(prop.name) ?? false : false });
				return {
					...prop,
...
const getHttpResourceVerbImports = (verbOptions, output) => {
	...
	return [
		...
		...parsedZodImports.filter((imp) => !isPrimitiveType(imp.name)).map((imp) => ({ name: getSchemaOutputTypeRef(imp.name) })),
		...body.imports,
...
const getParseExpression = (response, factory, output, responseTypeOverride) => {
	...
	return schemaName ? `${schemaName}.parse` : void 0;
};
/**
...
export function ${resourceName}(${implementationArgs}): HttpResourceRef<${resourceValueType}> {
  return ${resourceFactory}<${parsedDataType}>(() => applyOrvalRequestExtension(${request}, options)${resourceCallOptions});
}
`;
	return `/**
 * @experimental httpResource is experimental (Angular v19.2+)
...
const buildHttpResourceExtraFile = (verbOptions, outputPath, output, context, header) => {
	...
	const verbImports = Object.values(verbOptions).filter((verbOption) => isRetrievalVerb(verbOption.verb, verbOption.operationName, getClientOverride(verbOption))).flatMap((verbOption) => getHttpResourceVerbImports(verbOption, output));
	...
	const schemaImports = buildSchemaImportDependencies(output, schemaVerbImports, getHttpResourceRelativeSchemasPath(output, outputPath));
